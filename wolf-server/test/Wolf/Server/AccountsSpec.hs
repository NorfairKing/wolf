    ...
    describe "getAccounts" $ do
      ...
      it "gets the two accounts that were just stored" $ \env ->
        forAll (genValid `suchThat` uncurry ((/=) `on` accountUsername)) $ \(acc1, acc2) -> do
          ...
          case muuids of
            Nothing ->
              expectationFailure "Should have been able to add the accounts."
            Just (u1, u2) -> do
              as <-
                withEnv env $ do
              ...
              as `shouldBe`
                M.fromList
                  [(accountUsername acc1, u1), (accountUsername acc2, u2)]
    ...
    describe "lookupAccountUUID" $
      it "succesfully looks up newly registered accounts" $ \env ->
        forAllValid $ \un -> do
          ...
          case mm of
            (Nothing, _) ->
              expectationFailure "Should not have failed to add the account."
            (Just _, Nothing) ->
              expectationFailure

...
reviewSingle s = do
  ...
  case yn of
    No -> do
      ...
      let message =
            unwords
              ...
              , case oldAliases ++ entrySuggestionNewAliases (suggestionData s) of
    ...
    Yes -> do
      newAliases <-
        promptAboutAliases $ entrySuggestionNewAliases $ suggestionData s
      when (null $ oldAliases ++ newAliases) $
        liftIO $
        die "Cannot have a person without an index, choose at least one alias."
      ...
      commitMessage <-
        case entrySuggestionLikelyRelevantPerson $ suggestionData s of
          ...
          Just (personUuid, _) -> do
            origIndex <- getIndexWithDefault
            let displayName =
                  case reverseIndexLookup personUuid origIndex ++ newAliases of
                    [] -> "an un-aliased person"
                    (a:_) -> a
        ...
        pure $
          ...
          let yellow = colored [SetColor Foreground Dull Yellow]
           in case mentry of
                ...
                Just entry ->
                  [yellow "Entry: ", textReport $ entryContents entry]
  let infoReport =
        unlinesReport
          [ white "Suggestion:"
...
promptAboutAliases aliases = do
  ...
  case yn of
    ...
    No -> do
      ...
      liftIO $
        case er of
          EditingFailure err -> die $ "Failed to edit; " <> T.unpack err
          EditingSuccess ->
            (map alias .
             filter (not . T.null) . filter (not . T.isPrefixOf "#") . T.lines) <$>
  ...
  case mEntry of
    ...
    Just entry -> do
      ...
      case yn of
        ...
        No -> do
          tef <- tmpEntryFile
          liftIO $ SB.writeFile (toFilePath tef) $ entryContentsBS entry

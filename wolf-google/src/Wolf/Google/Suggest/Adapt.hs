                ...
                case v of
                  ...
                  (PMap vs_) ->
                    case lookup key1 vs_ of
                      Just (PVal v_) -> [personPropertyValueContents v_]
                      Nothing -> []
                      Just (PList _) -> []
                      Just (PMap _) -> []
            Just (PMap vs_) ->
  ...
  case personEntryProperties pe of
    ...
    PMap kvs ->
      case lookup key kvs of
        ...
        Just (PMap vs) ->
          mapMaybe (fmap personPropertyValueContents . onValue . snd) vs
  where
    onValue (PVal v) = Just v
    onValue _ = Nothing

adaptToPerson ::
     [Alias] -> PersonEntry -> PersonContext -> Maybe ([Alias], PersonProperty)

  ...
  describe "export" $ do
              ...
              forAllValid $ \repo -> do
                repo' <-
                  runData sets $ do
                    ...
                    importRepo repo
                    runCautiousT exportRepo
                (func <$> repo') `shouldBe` CautiousWarning mempty (func repo)
      ...
      forAll gen $ \sets ->
        forAllValid $ \(pu, entry) ->
          uncheckedRepoTest sets $ \repo ->
            repo
              {repoPersonEntries = M.insert pu entry $ repoPersonEntries repo}
      ...
      forAll gen $ \sets ->
        forAllValid $ \pu ->
          forAllValid $ \nu ->
            ...
            addNoteToIndexRepo nu . addPerson pu (NoteIndex $ S.singleton nu)
    ...
    it "fixes when person mentions non-existent note" $ \gen ->
      forAll gen $ \sets ->
        forAllValid $ \nu ->
          forAllValid $ \pu ->
            uncheckedRepoTest sets $ addPerson pu $ NoteIndex $ S.singleton nu

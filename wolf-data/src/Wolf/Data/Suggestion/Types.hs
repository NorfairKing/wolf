...
parseSuggestionTypeText t =
  case parseRelDir $ T.unpack t of
    Nothing -> fail "failed to parse SuggestionType, not a valid Path Rel Dir"
...
instance FromJSON a => FromJSON (Suggestion a) where
  parseJSON =
    withObject "Suggestion" $ \o ->
      Suggestion <$> o .: "suggestor" <*> o .: "reason" <*> o .: "timestamp" <*>
      o .: "data"
...
instance ToJSON AliasSuggestion where
  toJSON AliasSuggestion {..} =
    object ["person" .= aliasSuggestionPerson, "alias" .= aliasSuggestionAlias]

data EntrySuggestion =
  EntrySuggestion
...
instance (Validity a, Hashable a) => Validity (SuggestionTypeRepo a) where
  validate SuggestionTypeRepo {..} =
    mconcat
      ...
      , check
          (M.null
             (M.intersection suggestionTypeRepoUnused suggestionTypeRepoUsed))
...
instance ToJSON a => ToJSON (SuggestionTypeRepo a) where
  toJSON SuggestionTypeRepo {..} =
    object
      ["unused" .= suggestionTypeRepoUnused, "used" .= suggestionTypeRepoUsed]
